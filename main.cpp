...
uint8_t isAllowedExtension(string extension){
...
void registerNotCopiedFile(fs::path* lss, uint32_t& count, fs::path& path){
      lss[count] = path;
      count++;
}
...
int main(void){
     ...
     // por cada subcarpeta obtener y copiar los archivos terminados en .mp3 o .wav 
      ...
      fs::path contentsFolderPath = (fs::path) "Contents";
      if (!(fs::exists(contentsFolderPath))) { cerr << "ERROR: No existe la carpeta de origen" << endl; } 

      cout << endl;
      cout << "Copiando archivos." << endl;
      for (const auto& artistFolder: fs::directory_iterator(contentsFolderPath)){               // contents/artists
            for (const auto& sourceDownloadFolder: fs::directory_iterator(artistFolder)){       // contents/artists/source
                  for (const auto& file: fs::directory_iterator(sourceDownloadFolder)){         // contents/artists/source/file
                        ...
                        try{
                              ...
                              cout << "(*) Copiado exitosamente " << srcPath << endl;
